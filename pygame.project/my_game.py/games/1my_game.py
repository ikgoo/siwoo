...
# first_sound = pygame.mixer.Sound('E:\siwoo\pygame.project\my_game.py\sounds/0_angel_waltz_180.ogg') #소리
...
mob1 = mob_class.mob_class(screen,600,300,mob_killed,mob_image,0,0,0)
...
def fighting(charater,mob,charater_dam1,charater_dam2,mob_dam,charater_skills1,charater_skills2,mob_HP,charater_HP,mob_name):
    # charater_HP_font = game_font.render(">{0} HP:{1}".format(),True,(255,255,255))
    global mob_killed
    if mob_killed == 0:
        global running
        global no_mob
...
pygame.mixer.music.load(os.path.join(sound_path,"Tutorial.mp3"))
...
first_backgrond_sound = pygame.mixer.music.load(os.path.join(sound_path,"0_angel_waltz_180.ogg"))
...
while screen_num == 2:
    ...
    if test < 0:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_UP:
                    chacracter_to_y -= chacracter_speed
                if no_down == 0:
                    if event.key == pygame.K_DOWN:
                        chacracter_to_y += chacracter_speed 
    ...
    mob1.colliderect(charater_rect)
    mob1.colliderect(fence_down_rect)
    mob1.colliderect(fence_up_rect)
    mob1.colliderect(fence_left_rect)
    mob1.colliderect(fence_right_rect)
    if(mob1.colliderect_cha(charater_rect)):
        # mob_hit_effect(100,2000)
    ...
    if ishit == False and charater_rect.colliderect(mob_rect):
        ...
        chacracter_to_y = 0
        mob_hit_effect()
        fighting(charater,mob,20,30,10,"kimchissadagu","faxtfockuck",100,200,"super ultra strong guy")
